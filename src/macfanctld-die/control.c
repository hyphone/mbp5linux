...
struct sensor *sensor_TN0D = NULL;
...
void calc_fan()
{
	...
	if(sensor_TG0D != NULL)
	{
		...
		float fan_TG0D_speed =(normalized_temp * fan_window);
	...
	// calc fan speed for TN0D

	if(sensor_TN0D != NULL)
	{
		float temp_window = temp_TN0D_ceiling - temp_TN0D_floor;
		float normalized_temp =(sensor_TN0D->value - temp_TN0D_floor) / temp_window;
		float fan_TN0D_speed =(normalized_temp * fan_window);
		if(fan_TN0D_speed > fan_speed)
		{
			fan_speed = fan_TN0D_speed;
			fan_ctl = CTL_TN0D;
		}
	}
...
void scan_sensors()
{
	...
	if(sensor_count > 0)
	{
		...
		for(i = 0; i < sensor_count; ++i)		// for each label found
		{
			if(! sensors[i].excluded)
			{
				// try to find TC0D and TG0D and TN0D
				// if found, assign sensor_TC0D and sensor_TG0D for later use
				...
				else if(strcmp(sensors[i].name, "TN0D") == 0)
				{
					sensor_TN0D = &sensors[i];
				}
			}
